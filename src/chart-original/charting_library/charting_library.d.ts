...
/**
 * Mode to clear the marks on the chart-original.
 */
...
export interface AccountManagerColumnBase<TFormatterName extends StandardFormatterName | FormatterName> {
	 ...
	 * | `StandardFormatterName.Symbol` | It is used for a symbol field. It displays `brokerSymbol`, but when you click on a symbol the chart-original changes according to the `symbol` field. |
...
export interface AdditionalSymbolInfoField {
	...
	title: string;
	/** used to look up a property from the symbol info returned from the chart-original's datafeed */
	propertyName: string;
}
/**
...
export interface BrokerConfigFlags {
	 ...
	 * If this flag is set to `true`, then the chart-original displays trailing stop orders and a user can place a trailing stop order using Order Ticket.
	 * @default false
	 */
	 ...
	 * additional fields will be disabled in Order Ticket on the chart-original,
	 * and 'Modify' button will be hidden from the chart-original and in the Account Manager.
	 * @default true
	 */
...
/**
 * Saved chart-original data
 */
export interface ChartData {
	/** unique ID of the chart-original (may be `undefined` if it wasn't saved before) */
	id: string | undefined;
	/** name of the chart-original */
	name: string;
	/** symbol of the chart-original */
	symbol: string;
	/** resolution of the chart-original */
	resolution: ResolutionString;
	/** content of the chart-original */
	content: string;
}
/**
 * Meta information about a saved chart-original
 */
export interface ChartMetaInfo {
	/** unique ID of the chart-original. */
	id: number;
	/** name of the chart-original */
	name: string;
	/** symbol of the chart-original */
	...
	/** resolution of the chart-original */
	resolution: ResolutionString;
	/** UNIX time when the chart-original was last modified */
	timestamp: number;
}
/**
...
export interface ChartPropertiesOverrides extends StudyOverrides {
	...
	/**
	 * Main series chart-original style.
	 *
	...
	/**
	 * Sessions to display on the chart-original. Use `'extended'` to include pre- and post-market subsessions. See the [Extended Sessions](https://www.tradingview.com/charting-library-docs/latest/connecting_data/Extended-Sessions) guide for more info.
	 *
	 * @default 'regular'
	 */
...
/**
 * Chart template content. The properties of the chart-original that are saved/loaded when the library saves/loads a chart-original template.
 */
export interface ChartTemplateContent {
	...
	/**
	 * Series properties (for example chart-original style, etc).
	 */
	mainSourceProperties?: any;
	/**
	 * The version of the chart-original template.
	 */
	version?: number;
}
...
export interface ChartingLibraryWidgetOptions {
	/**
	 * The `container` can either be a reference to an attribute of a DOM element inside which the iframe with the chart-original will be placed or the `HTMLElement` itself.
	 *
	 * ```javascript
	...
	/**
	 * JavaScript object that implements the datafeed interface ({@link IBasicDataFeed}) to supply the chart-original with data. See [Connecting Data](https://www.tradingview.com/charting-library-docs/latest/connecting_data/connecting_data.md) for more information on the JS API.
	 *
	 * ```javascript
	...
	/**
	 * The default interval for the chart-original.
	 *
	 * Example:
	 * ```javascript
	 * interval: '1D',
	...
	/**
	 * The default symbol for the chart-original.
	 *
	 * Example:
	 * ```javascript
	 * symbol: 'AAPL',
	...
	/**
	 * Boolean value showing whether the chart-original should use all the available space in the container and resize when the container itself is resized.
	 * @default false
	 *
	 * ```javascript
	 * autosize: true,
	...
	autosize?: boolean;
	/**
	 * Setting this property to `true` will make the chart-original write detailed API logs into the browser console.
	...
	debug?: boolean;
	/**
	 * The array containing names of features that should be disabled by default. `Feature` means part of the functionality of the chart-original (part of the UI/UX). Supported features are listed [here](https://www.tradingview.com/charting-library-docs/latest/customization/Featuresets.md).
	 *
	 * Example:
	 * ```javascript
	...
	/**
	 * You can hide some drawings from the toolbar or add custom restrictions for applying them to the chart-original.
	 *
	...
	/**
	 * The array containing names of features that should be enabled by default. `Feature` means part of the functionality of the chart-original (part of the UI/UX). Supported features are listed [here](https://www.tradingview.com/charting-library-docs/latest/customization/Featuresets.md).
	 *
	 * Example:
	 * ```javascript
	...
	/**
	 * Boolean value showing whether the chart-original should use all the available space in the window.
	 * @default false
	 *
	 * ```javascript
	 * fullscreen: true,
	 ...
	 * ```
	 *
	 * * If you would like to host the library on a separate origin to the page containing the chart-original then please view the following guide: [Hosting the library on a separate origin](https://www.tradingview.com/charting-library-docs/latest/getting_started/Hosting-Library-Cross-Origin.md).
	 */
	library_path?: string;
	/**
	...
	/**
	 * JS object containing saved chart-original content.
	 * Use this parameter when creating the widget if you have a saved chart-original already.
	 * If you want to load the chart-original content when the chart-original is initialized then use `load()` method ({@link IChartingLibraryWidget.load}) of the widget.
	 */
	saved_data?: object;
	/**
	 * JS object containing saved chart-original content meta info.
	 */
	saved_data_meta_info?: SavedStateMetaInfo;
	/**
	 * You can hide some studies from the toolbar or add custom restrictions for applying them to the chart-original.
	 *
	 * @example
	 * ```javascript
	 * studies_access: {
	...
	/**
	 * Maximum amount of studies on the chart-original of a multichart layout. Minimum value is 2.
	 *
	 * ```javascript
	 * study_count_limit: 5,
	 * ```
	...
	/**
	 * Sets the default timeframe of the chart-original.
	 *
	 ...
	 *
	 * **Note**:
	 * When using a range the chart-original will still request data up to the current date. This is to enable scrolling forward in time once the chart-original has loaded.
	 */
	timeframe?: TimeframeOption;
	/**
	 * Default timezone of the chart-original. The time on the timescale is displayed according to this timezone.
	 ...
	 * ```
	 *
	 * **Remark**: If you want the chart-original to use all the available space use the `fullscreen` parameter instead of setting it to '100%'.
	 */
	width?: number;
	/**
	...
	user_id?: string;
	/**
	 * Set this parameter to `true` if you want the library to load the last saved chart-original for a user (you should implement [save/load](https://www.tradingview.com/charting-library-docs/latest/saving_loading/) first to make it work).
	 *
	 * ```javascript
	...
	/**
	 * This URL is used to send a POST request with binary chart-original snapshots when a user presses the snapshot button.
	...
	snapshot_url?: string;
	/**
	 * List of visible time frames that can be selected at the bottom of the chart-original. See [Time frame toolbar](https://www.tradingview.com/charting-library-docs/latest/ui_elements/Time-Scale.md#time-frame-toolbar) for more information. Time frame is an object containing the following properties:
	 *
	 * Example:
	 *
	 * ```javascript
	...
	/**
	 * Adds your custom CSS to the chart-original. `url` should be an absolute or relative path to the `static` folder.
	 *
	 * ```javascript
	...
	/**
	 * Changes the font family used on the chart-original including the [time scale](https://www.tradingview.com/charting-library-docs/latest/ui_elements/Time-Scale.md), [price scale](https://www.tradingview.com/charting-library-docs/latest/ui_elements/Price-Scale.md), and chart-original's pane.
	 * If you want to customize fonts outside the chart-original, for example, within [Watchlist](https://www.tradingview.com/charting-library-docs/latest/trading_terminal/Watch-List.md) or another widget,
	...
	/**
	 * An object that contains set/remove functions. Use it to save chart-original settings to your preferred storage (including server-side).
	 *
	 * Example:
	 * ```javascript
	 * settings_adapter: {
	...
	/**
	 * Set predefined custom theme color for the chart-original. Supported values are: `"light"` | `"dark"`.
	 *
	 * ```javascript
	 * theme: "light",
	 * ```
...
export interface CustomFormatters {
	...
	/** Used to format the date displayed over the timescale when hover over a chart-original */
...
/**
 * Describes a single execution. Execution is a mark on a chart-original that displays trade information.
 */
...
export interface ExportDataOptions {
	 ...
	 * User time is the time that user sees on the chart-original.
...
/**
 * Export data from the chart-original
 */
...
export interface Favorites<TChartTypeFavorites> {
	 ...
	 * this will match the chart-original UI in the English version.
	 *
	 * Example: `["Awesome Oscillator", "Bollinger Bands"]`.
	 */
	indicators?: string[];
	/**
	 * An array of chart-original types that are marked as favorite.
	 * The names of chart-original types are listed within the {@link ChartTypeFavorites} or {@link TradingTerminalChartTypeFavorites} type.
	 *
...
export interface HHistPreferences {
	...
	visible: boolean;
	/** Percentage width of the chart-original for which the histogram should occupy */
	percentWidth: number;
	 ...
	 * Whether the histogram will be shown on the left or right edge of the chart-original
	 */
...
export interface HiLoStylePreferences {
	/** Color for Hi Lo chart-original */
	color: string;
...
export interface IBrokerConnectionAdapterHost {
	...
	/**
	 * Call this method to notify the chart-original that it needs to update information after an order is added or changed.
...
export interface IChartWidgetApi {
	 ...
	 * );
	 * ```
	 * @returns A subscription object for new data loaded for the chart-original.
	 */
	onDataLoaded(): ISubscription<() => void>;
	/**
	 * Get a subscription object for the chart-original symbol changing.
	 *
	 * **Example**
	 * ```javascript
	 * widget.activeChart().onSymbolChanged().subscribe(null, () => console.log('The symbol is changed'));
	 * ```
	 * @returns A subscription object for the chart-original symbol changing.
	 */
	onSymbolChanged(): ISubscription<() => void>;
	/**
	 * Get a subscription object for the chart-original resolution (interval) changing. This method also allows you to track whether the chart-original's [date range](https://www.tradingview.com/charting-library-docs/latest/getting_started/glossary.md#date-range) is changed.
	 ...
	 *     });
	 * ```
	 * @returns A subscription object for the chart-original interval (resolution) changing.
	 */
	onIntervalChanged(): ISubscription<(interval: ResolutionString, timeFrameParameters: {
		timeframe?: TimeFrameValue;
	}) => void>;
	/**
	 * Get a subscription object for the chart-original's visible range changing.
	 *
	 * **Example**
	 * ```javascript
	 ...
	 * );
	 * ```
	 * @returns A subscription object for the chart-original's visible range changing.
	 */
	onVisibleRangeChanged(): ISubscription<(range: VisibleTimeRange) => void>;
	/**
	 * Get a subscription object for the chart-original type changing.
	 *
	 * **Example**
	 * ```javascript
	 * widget.activeChart().onChartTypeChanged().subscribe(
	 *     null,
	 *     (chartType) => console.log('The type of chart-original is changed')
	 * );
	 * ```
	 * @returns A subscription object for the chart-original type changing.
	 */
	onChartTypeChanged(): ISubscription<(chartType: SeriesType) => void>;
	/**
	 * Provide a callback function that will be called when chart-original data is loaded.
	 * If chart-original data is already loaded when this method is called, the callback is called immediately.
	 *
	 * **Example**
	 * ```javascript
	 ...
	 * ```
	 *
	 * @param callback A callback function called when chart-original data is loaded.
	 */
	dataReady(callback: () => void): boolean;
	/**
	 * Get a subscription object for the crosshair moving over the chart-original.
	 *
	 * **Example**
	 * ```javascript
	 ...
	 * );
	 * ```
	 * @returns A subscription object for the crosshair moving over the chart-original.
	 */
	...
	/**
	 * Scroll and/or scale the chart-original so a time range is visible.
	 *
	 * **Example**
	 * ```javascript
	...
	/**
	 * Change the chart-original's symbol.
	 *
	 * **Example**
	 * ```javascript
	 * widget.activeChart().setSymbol('IBM');
	 * ```
	 * Note: if you are attempting to change multiple charts (multi-chart-original layouts) at the same time with
	...
	/**
	 * Change the chart-original's interval (resolution).
	 *
	 * **Example**
	 * ```javascript
	...
	/**
	 * Change the chart-original's type.
	 *
	 * **Example**
	 * ```javascript
	 ...
	 * ```
	 *
	 * @param type A chart-original type.
	 * @param callback An optional callback function. Called when the chart-original type has changed and data has loaded.
	 */
	setChartType(type: SeriesType, callback?: () => void): void;
	/**
	 * Force the chart-original to re-request data.
	...
	/**
	 * Force the chart-original to re-request all bar marks and timescale marks.
	 *
	 * **Example**
	 * ```javascript
	...
	refreshMarks(): void;
	/**
	 * Remove marks from the chart-original.
	 *
	 * **Example**
	 * ```javascript
	...
	/**
	 * Get an array of IDs and name for all drawings on the chart-original.
	 *
	 * **Example**
	 * ```javascript
	...
	/**
	 * Get an array of IDs and names for all studies on the chart-original.
	 *
	 * **Example**
	 * ```javascript
	...
	/**
	 * Get the chart-original's price to bar ratio.
	 *
	 * **Example**
	 * ```javascript
	...
	/**
	 * Set the chart-original's price to bar ratio.
	 *
	 * **Example**
	 * ```javascript
	...
	/**
	 * Get the locked/unlocked state of the chart-original's price to bar ratio.
	 *
	 * **Example**
	 * ```javascript
	...
	/**
	 * Lock or unlock the chart-original's price to bar ratio.
	 *
	 * **Example**
	 * ```javascript
	...
	/**
	 * Maximize to its full size currently selected chart-original.
	 *
	 * **Example**
	 * ```javascript
	...
	maximizeChart(): void;
	/**
	 * Check if the chart-original is maximized or not.
	 *
	 * @returns `true` if maximized, `false` otherwise.
	 */
	isMaximized(): boolean;
	/**
	 * Restore to its initial size currently selected chart-original.
	 *
	 * **Example**
	 * ```javascript
	...
	/**
	 * Adds an indicator or a symbol for comparison to the chart-original.
	...
	/**
	 * Create a new anchored drawing. Anchored drawings maintain their position when the chart-original's visible range changes.
	 *
	 * **Example**
	 * ```javascript
	 ...
	 * ```
	 *
	 * @param position Percent-based x and y position of the new drawing, relative to the top left of the chart-original.
	 * @param options An options object for the new drawing.
	 */
	...
	/**
	 * Remove an entity (e.g. drawing or study) from the chart-original.
	 *
	 * **Example**
	 * ```javascript
	...
	/**
	 * Remove all drawings from the chart-original.
	 *
	 * **Example**
	 * ```javascript
	...
	/**
	 * Remove all studies from the chart-original.
	 *
	 * **Example**
	 * ```javascript
	 ...
	 * **Example**
	 * ```javascript
	 * const chart-original = widget.activeChart();
	 * chart-original.showPropertiesDialog(chart-original.getAllShapes()[0].id);`
	 * ```
	 *
	...
	/**
	 * Apply a study template to the chart-original.
	 *
	 * **Example**
	 * ```javascript
	...
	/**
	 * Create a new trading order on the chart-original.
	 *
	 * **Example**
	 * ```javascript
	...
	/**
	 * Creates a new trading position on the chart-original.
	 *
	 * **Example**
	 * ```javascript
	 * widget.chart-original().createPositionLine()
	 *     .onModify(function() {
	...
	/**
	 * Creates a new trade execution on the chart-original.
	 *
	 * **Example**
	 * ```javascript
	...
	/**
	 * Get an API object for interacting with the chart-original timezone.
	 */
	getTimezoneApi(): ITimezoneApi;
	/**
	 * Get an array of API objects for interacting with the chart-original panes.
	 *
	 * **Example**
	 * ```javascript
	...
	getPanes(): IPaneApi[];
	/**
	 * Export the current data from the chart-original.
	 *
	 * **Example**
	 * ```javascript
	 ...
	 * // Exports all available data from the chart-original
	 ...
	 * // Exports all displayed data on the chart-original
	...
	/**
	 * Check if the chart-original can be zoomed out using the {@link zoomOut} method.
	 *
	 * **Example**
	 * ```javascript
	 ...
	 * ```
	 *
	 * @returns `true` if the chart-original can be zoomed out.
	 */
	canZoomOut(): boolean;
	/**
	...
	zoomOut(): void;
	/**
	 * Enable or disable zooming of the chart-original.
	 *
	 * **Example**
	 * ```javascript
	...
	/**
	 * Enable or disable scrolling of the chart-original.
	 *
	 * **Example**
	 * ```javascript
	...
	/**
	 * Switch the chart-original to bar selection mode.
	 *
	 * **Example**
	 * ```javascript
	...
	/**
	 * Load and apply a chart-original template.
	 *
	 * @param templateName The name of the template to load.
	 */
	loadChartTemplate(templateName: string): Promise<void>;
	/**
	 * Get a readonly watched value that can be used to read/subscribe to the state of the chart-original's market status.
	 */
	marketStatus(): IWatchedValueReadonly<MarketStatus | null>;
	/**
	 * Set the time frame for this chart-original.
	 *
	 * **Note:** This action will set this chart-original as active in a multi-chart-original layout.
	 *
	 * **Example**
...
export interface IChartingLibraryWidget {
	...
	/**
	 * The library will call `callback` when the chart-original is ready to be used.
	 *
	 * @param callback A function that will be called when the chart-original is ready to be used.
	 */
	 ...
	 *   widget.chart-original().executeActionById("symbolSearch");
	 * });
	 *
	 * // F1
	...
	/**
	 * Get an API instance that can be used to interact with a chart-original.
	 *
	 * @param index Zero based index of the chart-original.
	...
	/**
	 * Set the symbol and resolution of the active chart-original.
	...
	/**
	 * Saves the chart-original state to a object. This method is part of the low-level save/load API.
	 *
	 * @param callback A function called with the chart-original state as the first argument.
	 */
	save(callback: (state: object) => void): void;
	/**
	 * Loads the chart-original state from a object. This method is part of the low-level save/load API.
	 *
	 * @param state A chart-original state object to load.
	...
	/**
	 * Get a list of chart-original descriptions saved to the server for the current user.
	 *
	 * @param callback A function called with an array of saved chart-original information as the first argument.
	 */
	getSavedCharts(callback: (chartRecords: SaveLoadChartRecord[]) => void): void;
	/**
	 * Load a saved chart-original from the server.
	 *
	 * @param chartRecord A chart-original information object (returned by {@link getSavedCharts}).
	 */
	loadChartFromServer(chartRecord: SaveLoadChartRecord): void;
	/**
	 * Save the current chart-original to the server.
	 *
	 * @param onComplete An optional callback function called when the chart-original is successfully saved.
	 * @param onFail An optional callback function called when the chart-original fails to save.
	 * @param options An optional object of options for saving the chart-original.
	 */
	saveChartToServer(onComplete?: EmptyCallback, onFail?: EmptyCallback, options?: SaveChartToServerOptions): void;
	/**
	 * Remove a saved chart-original from the server.
	 *
	 * @param chartId A chart-original ID from a {@link SaveLoadChartRecord} (returned by {@link getSavedCharts}).
	 * @param onCompleteCallback A callback function called when the chart-original is successfully saved.
	 */
	 ...
	 * ```
	 *
	 * @param callback A function called with the time and price of the location on the chart-original that triggered the context menu.
	...
	/**
	 * Get the symbol and interval of the active chart-original.
	 */
	...
	/**
	 * Apply overrides to the chart-original without reloading. See also {@link ChartingLibraryWidgetOptions.overrides}.
	 *
	 * @param overrides An object of overrides to apply to the chart-original.
	 */
	...
	/**
	 * Get an API object for interacting with the active chart-original.
	 *
	 * @returns An API object for interacting with the chart-original.
	 */
	activeChart(): IChartWidgetApi;
	/**
	 * Get the index of the active chart-original in the layout.
	 *
	 * @returns number.
	 */
	activeChartIndex(): number;
	/**
	 * Set which chart-original is currently active.
	 * It is recommended that this method is only used when linked to a user action
	 * which should change the active chart-original.
	 *
	 ...
	 * then this method will not change the active chart-original.
	 * @param index - index of chart-original to set as the active chart-original. Index is zero-based.
	 */
	...
	chartsCount(): number;
	/**
	 * Get the current chart-original layout type.
	 *
	 * @returns A string representation of the current layout type. E.g. `'2h'` for two charts split vertically.
	 */
	layout(): LayoutType;
	/**
	 * Set the current chart-original layout type.
	 *
	 * @params layout A string representation of the new layout type. E.g. `'2h'` for two charts split vertically.
	 */
	setLayout(layout: LayoutType): void;
	/**
	 * Get the name of the current chart-original layout. The return value will be `undefined` if the current layout has not been saved.
	 *
	 * @returns A string of the name of the current chart-original layout.
	 */
	layoutName(): string;
	/**
	 * Change the theme of the chart-original.
	 *
	...
	/**
	 * Get the current theme of the chart-original.
	 *
	 * **Example**
	 * ```javascript
	...
	getTheme(): ThemeName;
	/**
	 * Create a snapshot of the chart-original and upload it to the server.
	...
	takeScreenshot(): void;
	/**
	 * Create a snapshot of the chart-original and return it as a canvas.
	 *
	...
	/**
	 * Set the chart-original into fullscreen mode (if it isn't already).
	 */
	startFullscreen(): void;
	/**
	 * Set the chart-original into non-fullscreen mode (if it isn't already).
	 */
	exitFullscreen(): void;
	/**
	 ...
	 *
	 * An example of an acceptable use-case would be reusing a chart-original when switching
	 ...
	 * object that can be used to read/watch the current supported chart-original types
	 * ({@link SeriesType}) for an active chart-original.
	 *
	 * The chart-original type is returned as a number.
	 * You can see which number corresponds to which chart-original type in the
	 ...
	 * This can only be accessed when the chart-original is ready to be used. ({@link onChartReady})
	 *
	 ...
	 * be displayed within the legend for the main series of each chart-original.
	 *
	 * This can only be accessed when the chart-original has been created. ({@link headerReady})
	 *
...
export interface ICustomSymbolStatusApi {
	 ...
	 * be retrieved for a chart-original via the {@link IChartWidgetApi.symbol} method.
	 *
...
export interface IDatafeedChartApi {
	...
	/**
	 * This function is called if configuration flag supports_time is set to true when chart-original needs to know the server time.
	...
	/**
	 * This function is called when the chart-original needs a history fragment defined by dates range.
	 *
	 * @param symbolInfo A SymbolInfo object
	 * @param resolution Resolution of the symbol
...
export interface IExternalSaveLoadAdapter {
	 ...
	 * @returns Array of chart-original meta information
	 */
	getAllCharts(): Promise<ChartMetaInfo[]>;
	/**
	 * Remove a chart-original.
	 * @param  id - Unique ID of the chart-original (see {@link getAllCharts})
	 */
	removeChart<T extends number | string>(id: T): Promise<void>;
	/**
	 * Save the chart-original
	 * @param  {ChartData} chartData - Chart description data
	 * @returns unique ID of the chart-original
	 */
	saveChart(chartData: ChartData): Promise<string>;
	/**
	 * Load the chart-original from the server
	 * @param  {number} chartId - Unique ID of the chart-original to load (see {@link getAllCharts})
	 * @returns chart content contained in the `content` field when saving the chart-original ({@link ChartData})
	 */
	...
	/**
	 * Load a chart-original template from the server
	 *
	 * @param templateName The name of the template.
	 *
	 * @returns The chart-original template content.
	 */
	getChartTemplateContent(templateName: string): Promise<ChartTemplate>;
	/**
	 * Get names of all saved chart-original templates.
	 *
	 * @returns An array of names.
	 */
	getAllChartTemplates(): Promise<string[]>;
	/**
	 * Save a chart-original template.
	 *
	...
	/**
	 * Remove a chart-original template.
	 *
	 * @param templateName The name of the template.
	 */
...
export interface ILineDataSourceApi {
	...
	/**
	 * Can the drawing be saved in the chart-original layout
	 * @returns `true` when the drawing can be saved
	 */
	isSavingEnabled(): boolean;
	/**
	 * Enables or disables saving of the drawing in the chart-original layout (see `disableSave` option of `createMultipointShape`).
	 * @param  {boolean} enable - if `true`, the drawing can be saved to the chart-original
	 */
	...
	/**
	 * Places the drawing on top of all other chart-original objects.
	 */
	bringToFront(): void;
	/**
	 * Places the drawing behind all other chart-original objects.
	 */
	sendToBack(): void;
	/**
	 ...
	 * a single point to be 10% top the left edge of the chart-original and 10% from the top edge.
	 *
...
/**
 * Allows you to select entities ([drawings](https://www.tradingview.com/charting-library-docs/latest/ui_elements/Drawings) and [indicators](https://www.tradingview.com/charting-library-docs/latest/ui_elements/indicators/)) on the chart-original. Consider the following example:
 *
 * ```js
 * var chart-original = tvWidget.activeChart();
 * // Prints all selection changes to the console
 * chart-original.selection().onChanged().subscribe(null, s => console.log(chart-original.selection().allSources()));
 * // Creates an indicator and saves its ID
 * var studyId = chart-original.createStudy("Moving Average", false, false, { length: 10 });
 * // Adds the indicator to the selection ([<id>] is printed to the console)
 * chart-original.selection().add(studyId);
 * // Clears the selection ([] is printed to the console)
 * chart-original.selection().clear();
 * ```
 *
 * #### Multiple Selection
 *
...
export interface ISeriesApi {
	...
	/** Places main series on top of all other chart-original objects */
	bringToFront(): void;
	/** Places main series behind all other chart-original objects */
	sendToBack(): void;
	/** Value that is returned when a study is created via API */
	entityId(): EntityId;
	/** Returns properties for a specific chart-original style */
	chartStyleProperties<T extends ChartStyle>(chartStyle: T): SeriesPreferencesMap[T];
	/** Sets properties for a specific chart-original style */
...
export interface IStudyApi {
	...
	setVisible(visible: boolean): void;
	/**
	 * Move the study visually in front of all other chart-original objects.
	 */
	bringToFront(): void;
	/**
	 * Move the study visually behind of all other chart-original objects.
	 */
	sendToBack(): void;
	/**
	 ...
	 * Only applicable to multi-chart-original layouts (Trading Platform).
	 */
...
export interface ITimeScaleApi {
	...
	/**
	 * Users will be notified every time `barSpacing` value is changed. This typically occurs when zooming in/out on the chart-original.
	 * This is to detect when the chart-original has been zoomed in/out
	 */
	barSpacingChanged(): ISubscription<(newBarSpacing: number) => void>;
	/**
	 * Users will be notified every time `rightOffset` value is changed.
	 * This is to detect when the chart-original has been scrolled left/right.
	 */
	...
	rightOffset(): number;
	/** Returns the current width of the chart-original in pixels */
	width(): number;
...
export interface LibrarySymbolInfo {
	 ...
	 * Will be displayed in the chart-original legend for this symbol.
	 */
	description: string;
	/**
	...
	/**
	 * List of holidays for this symbol. These dates are not displayed on the chart-original.
	 ...
	 * The name will be displayed in the chart-original legend for this symbol.
	 *
	 * @example "NYSE"
	 */
	exchange: string;
	 ...
	 * - `c` - the symbol supports only close, it's displayed on the chart-original using line-based styles only
	 * @default 'ohlcv'
	 */
...
/**
 * Anchored (fixed) drawing point position as a percentage from the top left of a chart-original.
 * For example `{ x: 0.5, y: 0.5 }` for the centre of the chart-original.
 */
export interface PositionPercents {
	/**
	 * Position as a percentage from the left edge of the chart-original.
	 */
	x: number;
	/**
	 * Position as a percentage from the top edge of the chart-original.
	 */
	y: number;
}
...
export interface RawStudyMetaInfoBase {
	...
	/** Short description of the study. Will be displayed on the chart-original */
...
export interface SaveChartToServerOptions {
	/** Name of chart-original */
	chartName?: string;
	/** Default chart-original name */
	defaultChartName?: string;
}
export interface SaveLoadChartRecord {
	/** Unique id for chart-original */
	id: string;
	/** Name of chart-original */
	name: string;
	/** URL of chart-original image */
	image_url: string;
	/** Modified ISO number for chart-original */
	modified_iso: number;
	/** Short symbol name */
	short_symbol: string;
	/** Chart interval */
...
/**
 * Style preferences map for the chart-original types
 */
...
/**
 * Options for setting a chart-original's resolution.
 */
export interface SetResolutionOptions {
	...
	dataReady?: () => void;
	/**
	 * A boolean flag. Allows to disable making the current chart-original active in the layout.
	 */
	doNotActivateChart?: boolean;
}
/**
 * Options for setting a chart-original's symbol.
 */
 ...
 * of the range specified being ignored and the timestamp of the latest bar on the chart-original
 * being used instead.
 */
...
export interface SingleBrokerMetaInfo {
	 ...
	 *
	 * @example
	 * if you have field `additionalType` in orders and you want the chart-original to show a notification when it is changed, you should set:
	 * ```javascript
...
export interface StudyInputBaseInfo {
	...
	/** if true, then user will be asked to confirm input value before indicator is added to chart-original. Default value is false. */
...
export interface StudyLinePlotPreferences extends StudyPlotBasePreferences {
	...
	trackPrice: boolean;
	/**
	 * If defined, defines the number of bars to plot on chart-original.
	 */
...
export interface StudyStylesInfo {
	...
	/**
	 * Size of characters on the chart-original. Possible values are: `auto`, `tiny`, `small`, `normal`, `large`,`huge`. Applicable to `chars` and `shapes` plot types.
	 */
...
export interface SubscribeEventsMap {
	 ...
	 * @param  {boolean} isHidden - is the chart-original header currently hidden
	 */
	toggle_header: (isHidden: boolean) => void;
	/**
	 * Chart / Study Properties dialog is shown
	 * @param  {EditObjectDialogEventParams} params - meta information about the dialog type and the title of the chart-original or study
	 */
	edit_object_dialog: (params: EditObjectDialogEventParams) => void;
	/**
	 * New chart-original is about to be loaded
	 * @param  {object} savedData - chart-original data about to be loaded
	 */
	...
	/**
	 * A drawing has been added to a chart-original.
	...
	/**
	 * An indicator has been added to a chart-original.
	...
	/**
	 * User has changed the chart-original. `Chart change` means any user action that can be undone.
	 ...
	 * @param  {LayoutType} newLayoutType - whether the layout is single or multi-chart-original
	 */
	...
	/**
	 * Active chart-original has changed
	 * **Note:** this event is only applicable to Trading Platform.
	 * @param  {number} chartIndex - index of the active chart-original
	 */
...
/**
 * Definition of visible timeframes that can be selected at the bottom of the chart-original
 * @example
 * ```javascript
 * { text: "3y", resolution: "1W", description: "3 Years", title: "3yr" }
 * ```
...
export interface TradingTerminalWidgetOptions extends Omit<ChartingLibraryWidgetOptions, "enabled_features" | "disabled_features" | "favorites"> {
	/**
	 * The array containing names of features that should be disabled by default. `Feature` means part of the functionality of the chart-original (part of the UI/UX). Supported features are listed [here](https://www.tradingview.com/charting-library-docs/latest/customization/Featuresets).
	 *
	 * Example:
	 * ```javascript
	 * disabled_features: ["header_widget", "left_toolbar"],
	...
	/**
	 * The array containing names of features that should be enabled by default. `Feature` means part of the functionality of the chart-original (part of the UI/UX). Supported features are listed [here](https://www.tradingview.com/charting-library-docs/latest/customization/Featuresets).
	 *
	 * Example:
	 * ```javascript
	 * enabled_features: ["move_logo_to_main_pane"],
	...
	/**
	 * Settings for the widget panel on the right side of the chart-original.
	 * Watchlist, news, details and data window widgets on the right side of the chart-original can be enabled using the `widgetbar` field in Widget constructor
	 */
 ...
 * This is used when `includeUserTime: true` is defined in `exportData` to add user time (aka time that is displayed to the user on the chart-original) to exported data.
 */
...
"items_favoriting" | 
/** Can be disabled to forbid storing chart-original properties to the localstorage while allowing to save other properties. The other properties are favorites in the Advanced Charts and Watchlist symbols and some panels states in the Trading Platform @default true @default true */
"save_chart_properties_to_local_storage" | 
/** Add the volume indicator upon initialisation of the chart-original @default true */
...
/** Keeps the dialogs within the chart-original @default true */
...
/** Allows chart-original scrolling @default true */
"chart_scroll" | 
/** Allows chart-original zooming @default true */
"chart_zoom" | 
/** If enabled, the chart-original handles horizontal pointer movements on touch screens. In this case the webpage is not scrolled. If disabled, the webpage is scrolled instead. Keep in mind that if the user starts scrolling the chart-original vertically or horizontally, scrolling is continued in any direction until the user releases the finger @default true */
"horz_touch_drag_scroll" | 
/** If enabled, the chart-original handles vertical pointer movements on touch screens. In this case the webpage is not scrolled. If disabled, the webpage is scrolled instead. Keep in mind that if the user starts scrolling the chart-original vertically or horizontally, scrolling is continued in any direction until the user releases the finger @default true */
"vert_touch_drag_scroll" | 
/** If enabled, chart-original scrolling with horizontal mouse wheel is enabled @default true */
"mouse_wheel_scroll" | 
/** If enabled, chart-original scrolling with left mouse button pressed is allowed @default true */
...
/** Disables minimum chart-original width limitation @default false */
"no_min_chart_width" | 
/** Prevents scrolling to the left of the first historical bar @default false */
"fix_left_edge" | 
/** Prevents changing visible time area on chart-original resizing @default false */
"lock_visible_time_range_on_resize" | 
/** If disabled, adding a new bar zooms out the chart-original preserving the first visible point. Otherwise the chart-original is scrolled one point to the left when a new bar comes @default true */
...
/** By default many chart-original layouts can be saved with the same name. If this feature is enabled then the library will prompt to confirm overwriting chart-original layouts with the same name when saving, renaming, or cloning ("Save as") @default false */
"confirm_overwrite_if_chart_layout_with_name_exists" | 
/** By default the chart-original will request a small (fixed) number of bars for the initial data request when the chart-original is first created. If this feature is enabled then the library will rather calculate the request size based on the amount of bars which will be visible on the chart-original @default false */
...
/** Removes the header from the chart-original @default true */
"header_widget" | 
...
/** Hides the chart-original type button from the header @default true */
"header_chart_type" | 
...
/** Adds a 2px padding to the chart-original @default true */
...
"left_toolbar" | 
/** Relates to the navigation buttons at the bottom of the chart-original @default true */
"control_bar" | 
/** Hide the timeframe toolbar at the bottom of the chart-original @default true */
"timeframes_toolbar" | 
...
/** Removes the eye button that hides/shows a chart-original @default true */
"show_hide_button_in_legend" | 
/** Removes the cog icon for accessing chart-original/indicator's settings @default true */
...
"context_menus" | 
/** Doesn't display the context menu when right clicking on the chart-original @default true */
"pane_context_menu" | 
...
"legend_context_menu" | 
/** Displays the settings button in the bottom right corner of the chart-original @default true */
...
/** Adds High-low option to chart-original style controls @default true */
"chart_style_hilo" | 
/** Enables last price line and price axis label on High-low chart-original style @default false */
...
/** Hide the interval (D, 2D, W, M, etc.) in the chart-original legend and the data window @default false */
"hide_resolution_in_legend" | 
/** Hide unresolved symbols in the chart-original legend and the data window @default false */
"hide_unresolved_symbols_in_legend" | 
/** On touch device show the zoom and move buttons at the bottom of the chart-original @default false */
"show_zoom_and_move_buttons_on_touch" | 
/** Hide the optional symbol input value from the indicator's legend if 'Main chart-original symbol' option is selected @default true */
...
/** Show the option to specify the default right margin in percentage within chart-original settings dialog @default false */
...
/** Enable saving/loading of chart-original templates. */
"chart_template_storage" | 
/**
 * When chart-original data is reset, then re-request data for just the visible range (instead of the entire range of the existing data loaded).
 * @default false
 */
...
/**
 * Description of each field of exported data from the chart-original
 */
 ...
 * fallback to the timestamp of the latest bar on the chart-original.
 */
...
/** Shows trading notifications on the chart-original @default true */
...
"broker_button" | 
/** Order Panel is visible when the chart-original opens @default false */
...
/** Hide the right_toolbar when initialising the chart-original. Can be expanded using the widgetBar API {@link IWidgetbarApi}  @default false */
"hide_right_toolbar" | 
